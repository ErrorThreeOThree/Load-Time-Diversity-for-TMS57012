...
/* USER CODE END */
    ...
    if ((g_sciTransfer[0].mode & SCI_TX_INT) != 0)
    {
        /* we are in interrupt mode */
        
        g_sciTransfer[0].length = length;
        g_sciTransfer[0].data   = data;

        /* start transmit by sending first byte */        
        sci->TD     = *g_sciTransfer[0].data++;
...
/* USER CODE END */
    ...
    g_sciTransfer[0].mode |= (flags & SCI_TX_INT);
